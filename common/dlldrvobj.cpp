...
CDllDrvObj::CDllDrvObj()
{
	m_instanceCounter = 0;
#ifdef WIN32
	m_objMutex = CreateMutex( NULL, true, CANAL_DLL_TELLSTICKOBJ_OBJ_MUTEX );
#else
	pthread_mutex_init( &m_objMutex, NULL );
#endif

	// Init the driver array
	for ( int i = 0; i<CANAL_TELLSTICK_DRIVER_MAX_OPEN; i++ ) {
...
CDllDrvObj::~CDllDrvObj()
{
	...
	for ( int i = 0; i<CANAL_TELLSTICK_DRIVER_MAX_OPEN; i++ ) {
		...
		if ( NULL != m_drvObjArray[ i ] ) {
			...
			CTellstickObj *pdrvObj =  getDriverObject( i );
			if ( NULL != pdrvObj ) { 
				pdrvObj->close();	
...
long CDllDrvObj::addDriverObject( CTellstickObj *pdrvObj )
{
	long h = 0;
...
CTellstickObj * CDllDrvObj::getDriverObject( long h )
{
	long idx = h - 1681;

	// Check if valid handle
	if ( idx < 0 ) return NULL;
	if ( idx >= CANAL_TELLSTICK_DRIVER_MAX_OPEN ) return NULL;
...
void CDllDrvObj::removeDriverObject( long h )
{
	...
	// Check if valid handle
	if ( idx < 0 ) return;
	if ( idx >= CANAL_TELLSTICK_DRIVER_MAX_OPEN ) return;
