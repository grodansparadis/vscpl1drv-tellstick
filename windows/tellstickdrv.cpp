//
// tellstickdrv.cpp : Defines the entry point for the DLL application.
//
...
#endif
{
	...
	CTellstickObj *pdrvObj = new CTellstickObj();
	if ( NULL != pdrvObj ) {
...
#endif
{
	CTellstickObj *pdrvObj =  theApp->getDriverObject( handle );
...
#else
extern "C" int CanalBlockingSend( long handle, PCANALMSG pCanalMsg, unsigned long timeout  )
#endif
{
	CTellstickObj *pdrvObj =  theApp->getDriverObject( handle );
	if ( NULL == pdrvObj ) return 0;
...
#endif
{
	...
	return ( pdrvObj->readMsgBlock( pCanalMsg, timeout ) ? CANAL_ERROR_SUCCESS : CANAL_ERROR_GENERIC );
}
...
#else
extern "C" const char * CanalGetDriverInfo( void )
#endif
{
